-- FORCE CLEANUP ALL FUNCTIONS - STRONG CLEANUP
-- This script will completely remove ALL functions and recreate them cleanly

-- ===== STEP 1: FORCE DROP ALL FUNCTIONS =====
-- This is a nuclear option - removes everything

DO $$ 
DECLARE 
    func_record RECORD;
BEGIN
    -- Drop ALL functions with these names regardless of parameters
    FOR func_record IN 
        SELECT 
    ...
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.func_name || '(' || func_record.func_args || ') CASCADE';
    END LOOP;
END $$;

-- ===== STEP 2: VERIFY ALL FUNCTIONS ARE GONE =====
SELECT 'Functions Removed' as status,
...
-- ===== STEP 4: CREATE OPTIMIZED FUNCTIONS =====
...
-- Show final status
SELECT 
    'FORCE CLEANUP COMPLETE!' as status,
    'All functions removed and recreated cleanly' as details;

-- Count total functions - should be exactly 14
SELECT 
    'Final Function Count' as info,
    COUNT(*) as total_functions,
    CASE 
        WHEN COUNT(*) = 14 THEN '✅ Perfect - Exactly 14 functions!'
        ELSE '❌ Still have issues - ' || COUNT(*) || ' functions found'
    END as status
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public'
