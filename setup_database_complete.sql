...
-- This script ensures all required database objects exist for the application

-- Enable required extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- 1. Fix missing columns in sales table
DO $$
BEGIN
...
-- 2. Add variation_id columns to all transaction tables
DO $$
BEGIN
    -- sale_items table
    IF NOT EXISTS (
...
-- 3. Create or replace the generate_sku function
...
-- 4. Create or replace the SKU generation trigger
...
-- 5. Create or replace the tableHasColumn function (used by the API)
...
-- 6. Verify all required objects exist
SELECT 
    'Tables' as object_type,
    table_name as object_name,
    'OK' as status
FROM information_schema.tables 
WHERE table_name IN ('sales', 'sale_items', 'products', 'categories', 'inventory', 'stock_movements', 'transfers', 'transfer_items', 'users', 'branches', 'product_variations')
