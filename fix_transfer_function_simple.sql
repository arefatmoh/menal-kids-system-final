-- Simplified transfer function that shows all available data
-- This version doesn't require inventory to exist and handles missing columns
...
DECLARE
    result JSON;
BEGIN
    -- Get all active products (even without inventory)
    WITH product_data AS (
        SELECT 
            ...
            COALESCE(i.branch_id, p_from_branch_id) as branch_id,
        ...
        WHERE p.is_active = true
          AND (p_from_branch_id IS NULL OR i.branch_id = p_from_branch_id OR i.branch_id IS NULL)
    ),
    variation_data AS (
        SELECT 
            p.id as product_id,
...
SELECT 'Testing simplified function:' as test_name;
SELECT get_transfer_fast_simple('branch1') as result;
