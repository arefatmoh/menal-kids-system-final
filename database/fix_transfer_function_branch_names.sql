-- Fix branch names in transfer function
-- Ensure branches are returned correctly for the frontend
...
BEGIN
    ...
    variation_data AS (
        ...
        INNER JOIN inventory i ON p.id = i.product_id AND pv.id = i.variation_id
        INNER JOIN branches b ON i.branch_id = b.id
        WHERE p.is_active = true 
    ...
    transfers_data AS (
        ...
        WHERE t.from_branch_id = p_from_branch_id
    ...
    branches_data AS (
        ...
        WHERE is_active = true
          AND id != p_from_branch_id  -- Exclude user's own branch
    )
...
-- Test the function to ensure branches are returned
SELECT 'Testing branch names for branch1:' as test_name;
SELECT json_extract_path_text(get_transfer_fast_simple('branch1'), 'branches') as branches_branch1;

SELECT 'Testing branch names for branch2:' as test_name;
