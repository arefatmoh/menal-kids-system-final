-- Menal Kids Shop Database Schema
-- PostgreSQL Database for Kids Clothing Shop Management System
...
-- Create inventory table (updated to reference variations)
...
CREATE TABLE stock_movements (
    ...
    quantity INTEGER NOT NULL,
    reason VARCHAR(500),
    reference_type VARCHAR(50), -- 'sale', 'transfer', 'adjustment', 'return'
    reference_id UUID,
    created_at TIMESTAMP DEFAULT NOW()
);
...
GROUP BY t.id, fb.name, tb.name, u1.full_name, u2.full_name;
...
SELECT 
    ...
    COUNT(DISTINCT i.id) FILTER (WHERE i.quantity <= i.min_stock_level) as low_stock_items,
    COUNT(DISTINCT i.id) FILTER (WHERE i.quantity = 0) as out_of_stock_items,
    COALESCE(SUM(s.total_amount) FILTER (WHERE DATE(s.created_at) = CURRENT_DATE), 0) as today_sales,
    COUNT(s.id) FILTER (WHERE DATE(s.created_at) = CURRENT_DATE) as today_transactions,
...
LEFT JOIN alerts a ON b.id = a.branch_id
...
COMMENT ON DATABASE menal_kids_shop IS 'Menal Kids Shop Management System Database';
COMMENT ON TABLE branches IS 'Store branches information';
COMMENT ON TABLE users IS 'System users (owners and employees)';
COMMENT ON TABLE categories IS 'Product categories with hierarchical support';
COMMENT ON TABLE products IS 'Product catalog';
COMMENT ON TABLE inventory IS 'Stock levels per branch per product';
COMMENT ON TABLE sales IS 'Sales transactions';
COMMENT ON TABLE sale_items IS 'Individual items in each sale';
COMMENT ON TABLE stock_movements IS 'All stock movements (in/out) with reasons';
COMMENT ON TABLE transfers IS 'Inter-branch transfer requests';
COMMENT ON TABLE transfer_items IS 'Items in each transfer';
COMMENT ON TABLE budgets IS 'Budget allocations per branch/category';
COMMENT ON TABLE expenses IS 'Actual expenses recorded';
COMMENT ON TABLE alerts IS 'System alerts for various conditions';
