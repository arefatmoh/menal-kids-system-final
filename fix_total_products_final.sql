-- Final Fix for Total Products Branch-Specific Issue
-- This script properly drops and recreates the function to ensure changes take effect

-- First, drop the existing function
DROP FUNCTION IF EXISTS get_dashboard_stats(VARCHAR(50));
...
SELECT '=== TESTING UPDATED FUNCTION ===' as test_header;

-- Test for branch1 (Franko) - should show 67
SELECT 'Branch1 (Franko) - Expected: 67, Actual:' as test, total_products FROM get_dashboard_stats('branch1');

-- Test for branch2 (Mebrathayl) - should show 93  
SELECT 'Branch2 (Mebrathayl) - Expected: 93, Actual:' as test, total_products FROM get_dashboard_stats('branch2');

-- Test for all branches (NULL) - should show 160
SELECT 'All Branches (NULL) - Expected: 160, Actual:' as test, total_products FROM get_dashboard_stats(NULL);

-- Verify the logic is working correctly
SELECT '=== VERIFICATION QUERIES ===' as verification_header;

-- Verify branch1 only has 67 products
SELECT 'Branch1 products count:' as check_type, COUNT(DISTINCT p.id) as count
FROM products p
JOIN inventory i ON i.product_id = p.id
WHERE p.is_active = TRUE AND i.branch_id = 'branch1';

-- Verify branch2 only has 93 products
SELECT 'Branch2 products count:' as check_type, COUNT(DISTINCT p.id) as count
FROM products p
JOIN inventory i ON i.product_id = p.id
WHERE p.is_active = TRUE AND i.branch_id = 'branch2';

-- Verify total unique products is 160
SELECT 'Total unique products:' as check_type, COUNT(DISTINCT p.id) as count
FROM products p
JOIN inventory i ON i.product_id = p.id
WHERE p.is_active = TRUE;

SELECT 'Function updated successfully! Please refresh your dashboard.' as completion_message;
